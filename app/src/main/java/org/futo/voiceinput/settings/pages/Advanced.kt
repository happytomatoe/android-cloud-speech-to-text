...
import org.futo.voiceinput.settings.SettingTextDataStore
...
fun AdvancedScreen(
    ...
    ScrollableList {
        ...
        // Cloud Backend Settings (Debug builds only)
        if (BuildConfig.DEBUG) {
            ...
            SettingToggleDataStore(
                "Use Test File Mode",
                USE_TEST_FILE,
                subtitle = "Enable for E2E testing (debug builds only)"
            )

            SettingTextDataStore(
                title = "Test File Path",
                setting = TEST_FILE_PATH,
                placeholder = "/data/user/0/org.futo.voiceinput/cache/test_audio.wav"
            )
        }
